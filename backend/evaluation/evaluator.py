"""
Highlight: an end-to-end agent evaluation framework

Core question: how do you evaluate an end-to-end agent?

Evaluation dimensions:
  1. Intent accuracy -- predicted vs labelled intent, scored as Accuracy / F1
  2. Response quality -- LLM-as-Judge, scoring relevance, accuracy,
     completeness and helpfulness
  3. End-to-end dialogue -- simulate full multi-turn conversations and rate
     the overall experience
  4. Regression testing -- compare against a stored baseline to catch decay

LLM-as-Judge is the key technique here: human labelling is expensive and
subjective, while an LLM judge is scalable and repeatable.
"""
import asyncio
import json
import logging
...
class QualityScores:
    ...
    accuracy:     float   # is the information correct
    completeness: float   # does it fully resolve the request
    helpfulness:  float   # can the user act on it
...
class EvalReport:
    ...
    regressions:      List[str]          # metrics that regressed against the baseline
...
class LLMJudge:
    ...
    Why an LLM instead of human raters?
    - Scalable: thousands of test cases evaluated automatically
    - Repeatable: the same input yields a stable score
    - Multi-dimensional: relevance, accuracy and the rest scored in one pass

    Caveat: an LLM judge carries its own bias. Recalibrate against human labels
    periodically.
    """

    JUDGE_PROMPT = """You are an expert customer-service quality assessor. Score the customer-service response below.

User question: {question}
Agent response: {response}
{context_section}

Score these four dimensions from 0.0 to 1.0 and return JSON:
- relevance: does the response address the user's question (0 = unrelated, 1 = fully on point)
- accuracy: is the information correct (0 = clearly wrong, 1 = fully correct)
- completeness: does it fully resolve the user's need (0 = not at all, 1 = completely)
- helpfulness: can the user act on it (0 = useless, 1 = very actionable)
...
Return JSON only, for example: {{"relevance": 0.9, "accuracy": 0.8, "completeness": 0.7, "helpfulness": 0.85}}"""
    ...
    async def judge(
        ...
        ctx_section = f"Background: {context}" if context else ""
...
class EndToEndEvaluator:
    ...
    The run:
      1. Evaluate intent recognition (accuracy / F1)
      2. Evaluate dialogue quality (LLM-as-Judge)
      3. Compare against the stored baseline (regression detection)
      4. Emit actionable improvement suggestions
    """
    ...
    async def run(
        ...
        dialog_cases:
          ...
          - multi turn:  [{"turns": ["first turn", "second turn", ...]}]
        """
        results: List[EvalResult] = []
        ...
        # 2. Dialogue quality (run the orchestrator, then score with the LLM judge)
        if dialog_cases:
    ...
    def _history_context(history: List[Dict[str, str]]) -> str:
        ...
        return "[Multi-turn evaluation history]\n" + "\n".join(lines)

    def _detect_regressions(self, current: Dict[str, float]) -> List[str]:
        """Compare against the previous run and flag metrics that fell by over 5%."""
        ...
        for metric, value in current.items():
            if metric in prev and prev[metric] > 0:
                ...
                if delta < -0.05:
                    regressions.append(
                        f"{metric}: {prev[metric]:.3f} -> {value:.3f} (down {abs(delta):.1%})"
                    )
        return regressions
    ...
    def _recommendations(
        ...
        if scores.get("intent_accuracy", 1.0) < 0.90:
            recs.append("Intent accuracy below 90%: add more few-shot examples, or more training data for the low-F1 intent categories")
        if scores.get("relevance", 1.0) < 0.75:
            recs.append("Low relevance: review the agent system_prompt and make sure the agent stays on the user's question")
        if scores.get("completeness", 1.0) < 0.75:
            recs.append("Low completeness: the agent may be stopping early; ask for a complete solution in the prompt")
        ...
        if not recs:
            recs.append("Every metric is within target; keep it up")
        return recs
...
# ── Built-in test cases (usable out of the box) ───────────────────────────────

DEFAULT_INTENT_CASES: List[IntentTestCase] = [
    IntentTestCase("When will my parcel arrive?",        "logistics"),
    IntentTestCase("Please cancel my order",             "request"),
    IntentTestCase("Your service is terrible!",          "complaint"),
    IntentTestCase("The app keeps throwing a 500 error", "technical_crash"),
    IntentTestCase("Why was I charged twice?",           "payment_issue"),
    IntentTestCase("Get me a human agent",               "human_handoff"),
    IntentTestCase("Hello",                              "greeting"),
    IntentTestCase("Change my email address",            "account"),
    IntentTestCase("Please issue an invoice",            "invoice"),
    IntentTestCase("How long until my refund arrives?",  "refund"),
    IntentTestCase("Login keeps returning 401",          "technical_login"),
]

DEFAULT_DIALOG_CASES: List[Dict[str, Any]] = [
    {"question": "My order #12345 has not arrived and it is overdue"},
    {"question": "The app keeps failing to log in with a 401"},
    {"question": "Why was I charged an extra $50 this month?"},
    {"question": "Please change my delivery address to 221B Baker Street, London"},
    {"turns": ["Hello, I would like a refund", "The order number is #12345", "How long until the refund arrives?"]},
]
