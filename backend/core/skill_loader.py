"""
SupportMesh Skill loader.

A Skill is a hot-loadable description of a business capability that supplements
an agent's system prompt. It is the right place for company scripts, handling
procedures, compliance boundaries and troubleshooting SOPs -- the kind of rules
operations needs to adjust quickly, without a code change.
"""
import json
import logging
...
@dataclass
class Skill:
    """Normalized view of one Skill, hiding Markdown/JSON file-format differences."""
    name: str
    description: str
    content: str
    path: str
    ...
    def matches(self, message: str, agent_type: Optional[str] = None) -> bool:
        ...
        - Empty agents: applies to every agent; otherwise only the listed ones.
        - Empty keywords: injected globally; otherwise only on a keyword hit.
        """
        if not self.enabled:
            return False
        ...
        return any(keyword_matches(keyword.lower(), lowered) for keyword in self.keywords)

    def to_prompt_block(self, max_chars: int = 3200) -> str:
        """Format as a text block ready to append to a system prompt, capped in length."""
    ...
    def to_summary(self) -> Dict[str, Any]:
        """Return an API-serializable summary, keeping the full text out of health checks."""
        return {
            "name": self.name,
...
class SkillManager:
    ...
    def load(self) -> List[Skill]:
        """Rescan the directory and load Skills; one bad file does not block the rest."""
        loaded: List[Skill] = []
        errors: List[str] = []

        if not self.root_dir.exists():
            logger.info(f"Skill directory not found, skipping load: {self.root_dir}")
            self._skills = []
            self._errors = []
            return []
        ...
        for path in self._discover_files(self.root_dir):
            try:
                ...
                msg = f"{path}: {ex}"
                errors.append(msg)
                logger.warning(f"Failed to load Skill: {msg}")
    ...
    def reload(self) -> List[Skill]:
        """Hot-reload entry point, invoked at runtime by the API."""
        return self.load()

    def prompt_for(self, message: str, agent_type: Optional[str] = None) -> str:
        """
        Build the Skill prompt for the current user request.

        Only matching Skills are injected, truncated by total length so they do not
        crowd out the main conversation context.
        """
        blocks: List[str] = []
        ...
        if not blocks:
            logger.debug(
                "No Skills matched: agent=%s message=%r",
                agent_type or "all",
                (message or "")[:80],
            )
            return ""
        ...
        return (
            "The following SupportMesh Skills apply to this request. "
            "Follow these business rules first; where they conflict with your system role, the system role and safety boundaries win.\n\n"
            + "\n\n".join(blocks)
        )

    def summary(self) -> Dict[str, Any]:
        """Return manager state, used by the /skills endpoint and for troubleshooting."""
        return {
    ...
    def _log_loaded_skills(self) -> None:
        """Print a prominent load report so startup and hot reloads are easy to verify."""
        lines = [
            "",
            "================ SupportMesh Skills Loaded ================",
    ...
    def _discover_files(self, root_dir: Path) -> Iterable[Path]:
        """Discover loadable files, preferring the per-directory SKILL.md convention."""
    ...
    def _load_json(self, path: Path) -> Optional[Skill]:
        ...
        if not isinstance(raw, dict):
            raise ValueError("A JSON Skill must be an object")
    ...
    def _load_text(self, path: Path) -> Optional[Skill]:
        ...
        # If the first heading is only the Skill name, drop it when building the
        # prompt to avoid repeating the name.
    ...
    def _split_front_matter(self, raw: str) -> tuple[Dict[str, Any], str]:
        """
        Parse the simple front matter at the top of a Markdown file.

        PyYAML is deliberately avoided here: not worth a runtime dependency for
        such a lightweight config format.
        """
        text = raw.lstrip()
