"""
SupportMesh customer-service system -- FastAPI entry point

Prints a bear-cookie banner on startup.
Every core component is built in the lifespan handler and configured by env vars.
"""
import asyncio
import logging
import os
...
async def lifespan(app: FastAPI):
    ...
    # Intent recognizer (the Orchestrator builds its own; this one is exposed to the Evaluator)
    ...
    # Skills: business capability notes loaded from disk at startup and injected
    # into the prompt when an agent calls the LLM.
    skills_dir = os.getenv("SUPPORTMESH_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills"))
    _skill_manager = SkillManager(
        root_dir=skills_dir,
        max_prompt_chars=int(os.getenv("SUPPORTMESH_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    _skill_manager.load()
    ...
    # MCP tool manager + RAG knowledge base (real retrieval over ChromaDB)
    ...
    logger.info(f"Knowledge base loaded: {await kb.doc_count_async()} chunks")

    def knowledge_fallback(params: Dict[str, Any], context: Optional[Dict[str, Any]], error: str):
        query = params.get("query", "")
        return [{
            "title": "Knowledge base unavailable",
            "content": f"The knowledge base is temporarily unavailable, so the search for \"{query}\" could not run. Please retry shortly or ask for a human agent.",
            "score": 0.0,
            "fallback": True,
            "error": error,
        }]
    ...
    logger.info("SupportMesh is ready")
    yield

    await _monitor.stop()
    if _memory is not None:
        await _memory.close()
    logger.info("SupportMesh has shut down")


# ── FastAPI ───────────────────────────────────────────────────────────────────
app = FastAPI(
    title="SupportMesh Customer Service",
    version="2.0.0",
    lifespan=lifespan,
    docs_url="/docs",
)
...
async def skills_summary():
    """Inspect the currently loaded Skills, to confirm hot reloads and debug parse errors."""
...
async def reload_skills():
    """Rescan the Skill directory at runtime, no restart required."""
    if _skill_manager is None:
        raise HTTPException(503, "Skills are not initialized")
...
async def chat(req: ChatRequest):
    """
    The main dialogue endpoint. The full flow:
      read memory -> recognize intent -> route to an agent -> execute -> write memory
    """
    ...
    # 1. Read the memory context
    mem_ctx = await _memory.get_context(req.user_id, conv_id, query=req.message)

    # 2. Build the orchestration request (history included, as intent-recognition context)
    history = [
    ...
    # 5. Update the user profile asynchronously (does not block the response)
...
async def _build_knowledge_context(message: str, intent=None, top_k: int = 3) -> tuple[str, bool]:
    """
    Build the RAG knowledge context for the main /chat path.

    Reuses MCPToolManager's query rewriting, parallel recall, reranking and fallback.
    """
    ...
    try:
        ...
        if not used:
            return "", False
        parts.append("Answer from the knowledge base content above wherever possible; fall back on general customer-service guidance only where it is insufficient.")
        return "\n".join(parts), True
    except Exception as ex:
        logger.warning(f"Failed to build the knowledge-base context: {ex}")
        return "", False


def _should_use_knowledge(message: str, intent=None) -> bool:
    """Skip small talk; only search the knowledge base for real questions, so RAG cannot derail the reply."""
    ...
    if intent_value in {
        ...
        return True
    greetings = {"hi", "hello", "hey", "good morning", "good afternoon", "good evening", "hi there", "thanks"}
    if msg in greetings:
        return False
    business_keywords = [
        "refund", "order", "tracking", "delivery", "shipping", "invoice", "charge", "payment", "bill", "subscription",
        "login", "error", "crash", "broken", "membership", "points", "account", "password", "address",
    ]
    return len(msg.split()) >= 4 or any(kw in msg for kw in business_keywords)
...
async def search(query: str, top_k: int = 5):
    """
    Demonstrates the retrieval chain: rewrite -> parallel recall -> rerank -> Top-K.
    Shows off the core of MCP tool calling.
    """
    if _tool_manager is None:
        raise HTTPException(503, "Service not ready")
...
class BatchDocInput(BaseModel):
    """Request body for bulk document import."""
    documents: List[DocInput]


class EvalIntentInput(BaseModel):
    """An intent-recognition test case."""
    message: str
    expected_intent: str
...
class EvalDialogInput(BaseModel):
    """A dialogue-quality test case: `question` for single turn, `turns` for multi-turn."""
...
class EvalRunInput(BaseModel):
    """An evaluation request. Falls back to the built-in cases when empty."""
...
async def add_knowledge(body: BatchDocInput):
    ...
    Documents are chunked automatically (500 characters each) and stored in ChromaDB,
    whose built-in embedding model vectorizes them.

    Example request body:
    ```json
    {
      "documents": [
        {"title": "Refund policy", "content": "Customers may request a no-questions-asked refund within 7 days of purchase..."},
    ...
    if tool is None:
        raise HTTPException(503, "The knowledge base is not initialized")
    ...
    return {"message": f"Imported {count} chunks", "added_chunks": count, "total_chunks": total}


@app.post("/knowledge/upload", tags=["Knowledge base"])
async def upload_knowledge(file: UploadFile = File(...)):
    """
    Import a knowledge-base document by file upload.

    Supported formats:
    - `.txt` / `.md`: the whole file becomes one document, filename as the title
    ...
    if filename.endswith(".json"):
        ...
        # txt / md: the whole file becomes a single document
...
async def knowledge_stats():
    """Knowledge-base statistics (total number of chunks)."""
    tool = _tool_manager._tools.get("knowledge_search") if _tool_manager else None
    if tool is None:
        raise HTTPException(503, "The knowledge base is not initialized")
    kb = tool.handler.__self__
...
async def _cli():
    print(BANNER)
    print("SupportMesh CLI -- type quit to exit\n")
    ...
    skill_manager = SkillManager(
        root_dir=os.getenv("SUPPORTMESH_SKILLS_DIR", str(pathlib.Path(_ROOT) / "skills")),
        max_prompt_chars=int(os.getenv("SUPPORTMESH_SKILLS_MAX_PROMPT_CHARS", "5000")),
    )
    skill_manager.load()
    ...
    while True:
        ...
        print(f"\nSupportMesh [{result.agent_type.value}]: {result.response}\n")
