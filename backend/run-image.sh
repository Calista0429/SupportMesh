...
CONTAINER_NAME="supportmesh-app"
...
show_help() {
    cat << EOF
SupportMesh Docker image run tool

Usage: ./run-image.sh [command] [options]

Commands:
    run             run the container (default mode)
    run-dev         run a development container
    run-test        run a test container
...
run_container() {
    ...
    if [ ! -f "$env_file" ]; then
        print_warn "Environment file $env_file not found, using defaults"
        env_file=""
    else
    ...
    case $mode in
        dev)
            print_info "Running a development container"
            default_ports="$default_ports -p 5678:5678"  # add the debug port
            restart_policy="no"
            ;;
        test)
            print_info "Running a test container"
            restart_policy="no"
            ;;
        prod)
            print_info "Running a production container"
            restart_policy="unless-stopped"
            ;;
        *)
            print_info "Running a standard container"
            ;;
    esac
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ "$detach" = true ]; then
            ...
            print_info "Follow logs with: ./run-image.sh logs"
        fi
    else
...
clean_container() {
    ...
    print_warn "Removing the container and its data volumes"

    read -p "Confirm? This deletes the container and its data (y/N): " -n 1 -r
    echo
