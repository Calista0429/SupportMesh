"""
Highlight: monitoring live agent performance

Core question: how do you use a Monitor to watch agents in production?

This module's answer:
  1. Live collection -- pull fresh stats from the Orchestrator and ToolManager
     every N seconds
  2. Anomaly detection -- Z-score over a sliding window, catching sudden shifts
  3. Routing feedback -- write agent success rate and latency back to the
     Orchestrator, whose _best_agent() adjusts routing weights accordingly
  4. Suggestions -- rule-based, actionable advice rather than platitudes
  5. Alerting -- log on threshold breach, plus an optional webhook
"""
import asyncio
import logging
import statistics
...
class Suggestion:
    ...
    title:       str
    detail:      str
    action:      str    # concrete steps to take
    priority:    int    # 1-10
...
class AnomalyDetector:
    """
    Anomaly detection based on a sliding-window Z-score.

    Z-score = |current - mean| / stddev
    Anything beyond `sensitivity` standard deviations counts as an anomaly.
    """
    ...
    def record(self, metric: str, value: float) -> Optional[Dict[str, Any]]:
        """Record one data point; return anomaly details, or None if it looks normal."""
...
class PerformanceMonitor:
    ...
    How it couples back to the Orchestrator:
      Monitor collects -> notices an agent's success rate dropping ->
      that agent's routing_score in Orchestrator.get_stats() drops ->
      _best_agent() starts routing around it

    That loop is what "monitoring live performance" actually buys you.
    """
    ...
    async def _collect(self) -> None:
        """
        Collect live agent and tool stats, detect anomalies, emit suggestions.

        The point: these stats are the very ones the Orchestrator and ToolManager
        update while serving requests, so the Monitor needs no extra instrumentation.
        """
        ...
        for tool_name, s in tool_stats.items():
            ...
            if cf >= 3:
                self._add_suggestion(Suggestion(
                    ...
                    detail=f"success rate {sr:.1%}, average latency {ms:.0f}ms, breaker: {s['circuit_state']}",
                    action="1. Check that the tool's upstream service is healthy\n2. Read the error logs\n3. Consider a longer timeout or a fallback strategy",
                    priority=9,
                ))
    ...
    def _routing_penalty(success_rate: float, avg_ms: float) -> float:
        """Turn live performance into a 0-0.9 routing penalty factor."""
        penalty = 0.0
    ...
    def _check_threshold(self, metric: str, value: float, label: str) -> None:
        ...
        if triggered:
            ...
            # Fire the webhook asynchronously so the collection loop is not blocked
            if self._webhook:
                asyncio.create_task(self._send_webhook(alert))

    def _generate_routing_suggestions(self, agent_stats: Dict[str, Any]) -> None:
        """
        Generate routing suggestions from live agent performance.
        This is the Monitor -> Orchestrator feedback loop made visible.
        """
        for agent_key, s in agent_stats.items():
            if s["success_rate"] < 0.85 and s["total"] > 10:
                self._add_suggestion(Suggestion(
                    ...
                    action=(
                        "The Orchestrator's _best_agent() has already lowered this agent's routing weight.\n"
                        "Suggested next steps:\n"
                        "  1. Review whether the system_prompt needs tightening\n"
                        "  2. Check whether this problem type exceeds the agent's ability\n"
                        "  3. Consider adding another instance of this agent type"
                    ),
                    priority=8,
                ))

    def _add_suggestion(self, s: Suggestion) -> None:
        # Deduplicate: do not append a suggestion whose title is already present
    ...
    def summary(self) -> Dict[str, Any]:
        """Return the current monitoring summary for the API layer to expose."""
        return {
            "agent_stats":   self._orchestrator.get_stats(),
            "tool_stats":    self._tool_manager.get_stats(),
